/* global importScripts, addEventListener, postMessage, ZipInflater, ZipDeflater, ZipEncryption, ZipDecryption */
...
(() => {
	...
	function init(message) {
		...
		task = {
			...
			decryption: options.inputEncrypted && new ZipDecryption(options.inputPassword, options.inputSigned),
			encryption: options.outputEncrypted && new ZipEncryption(options.outputPassword)
		};
	}
	...
	async function flush() {
		...
		if (task.inputEncrypted) {
			const result = await task.decryption.flush();
			if (!result.valid) {
			...
			if (task.inputSignature != dataViewSignature.getUint32(0)) {
				throw new Error(ERR_INVALID_SIGNATURE);
			}
		}
		...
		if (task.outputCompressed) {
			data = task.deflater.flush();
		}
		if (task.outputEncrypted) {
			data = await task.encryption.append(data);
			const result = await task.encryption.flush();
