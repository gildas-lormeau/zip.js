...
async function test() {
	...
	try {
		const bytes = new Uint8Array(await (await fetch(new URL("../data/malformed-aes-method.zip", import.meta.url))).arrayBuffer());
		...
		if (!error || error.message != zip.ERR_UNSUPPORTED_COMPRESSION) {
			throw new Error("expected " + zip.ERR_UNSUPPORTED_COMPRESSION + ", got " + (error && error.message));
		}
	} finally {
