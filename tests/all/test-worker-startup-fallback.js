...
// the file this URI names must NOT exist: loading it is what fails. The name is long because it is the only
// text we control in the failure the runtimes report, e.g. deno type-checks the module and prints a TS2307
// and a bare "error: Uncaught (in worker) Type checking failed." in the middle of an otherwise green run.
const MISSING_WORKER_URI = new URL("./../data/missing-worker-expected-by-test-worker-startup-fallback.js", import.meta.url).href;
