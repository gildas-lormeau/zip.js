...
[@zip.js/zip.js](../globals.md) / ZipFS
...
Unlike [ZipFS#entries](#entries), the directory itself is not included and removed entries leave no empty slot.
...
defaults to `true`; a [ZipWriter](ZipWriter.md) passed to [ZipDirectoryEntry#exportZip](ZipDirectoryEntry.md#exportzip) defaults
...
a large file without buffering it, use [ZipDirectoryEntry#importZip](ZipDirectoryEntry.md#importzip) with a seekable input, see
...
[ZipDirectoryEntry#addDirectory](ZipDirectoryEntry.md#adddirectory) are written. A zip file storing no directory entry therefore
