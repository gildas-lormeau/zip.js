...
[ZipWriterConstructorOptions#zipCrypto](#zipcrypto) option is set to `true`, except for such a folder or empty
...
[ZipWriterConstructorOptions#level](#level) set to `0` on that entry. EPUB asks for the same pair: OCF states
...
turn, i.e. [ZipWriterConstructorOptions#bufferedWrite](#bufferedwrite) is set automatically for them.
...
When the [ZipWriterConstructorOptions#passThrough](#passthrough) option passes the compression stage through,
...
The data is never compressed, so the [ZipWriterConstructorOptions#level](#level) option selects no codec, and
neither does the [ZipWriterAddDataOptions#compressionMethod](#compressionmethod) option: both describe how the data is
...
The entries with no content, e.g. the directories, ignore this option entirely. Setting the [ZipWriterConstructorOptions#password](#password) or the
