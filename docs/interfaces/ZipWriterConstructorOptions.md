...
The data is never compressed, so the [ZipWriterConstructorOptions#level](#level) and
[ZipWriterAddDataOptions#compressionMethod](#compressionmethod) options select no codec. They are still recorded in the
