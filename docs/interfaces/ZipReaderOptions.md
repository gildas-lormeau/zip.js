...
[ZipReaderOptions#checkLocalFilename](#checklocalfilename) to control the filename comparison on its own. An explicit
...
field, which is why it is left out below [ZipReaderOptions#strictness](#strictness) set to `"strict"`. This option
selects what is compared without changing whether a difference throws or warns, which
[ZipReaderOptions#checkLocalDirectory](#checklocaldirectory) decides. It is therefore the only way to obtain
...
the value of [ZipReaderOptions#checkLocalDirectory](#checklocaldirectory) when it is set, otherwise `true`
when [ZipReaderOptions#strictness](#strictness) is `"strict"` and `false` when it is `"balanced"` or
`"tolerant"`.
...
Since the encryption is undone, `"compressed"` needs the [ZipReaderOptions#password](#password) option and
...
The [ZipReaderOptions#checkAuthenticationCode](#checkauthenticationcode) option applies as well. The
[ZipReaderOptions#checkCrc32](#checkcrc32) option does not, since the CRC32 of the entry describes its
...
[ZipReaderOptions#checkLocalDirectory](#checklocaldirectory) to request or suppress the whole comparison explicitly, and
[ZipReaderOptions#checkLocalFilename](#checklocalfilename) to include or exclude the filename on its own.
